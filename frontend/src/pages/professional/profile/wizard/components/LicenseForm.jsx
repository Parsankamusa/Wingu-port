...
const LicenseForm = ({ data, onUpdate, onCompletionUpdate, errors, setErrors }) => {
  ...
  useEffect(() => {
    const calculateCompletion = () => {
      ...
      licenses.forEach(license => {
        // Required fields for each license entry
        const requiredFields = [
          license.license_type,
          license.issue_authority,
        ...
        totalCompleted += requiredFields.filter(field => 
  ...
  const validateForm = () => {
    ...
    licenses.forEach((license, index) => {
      ...
      if (!license.expiry_date) {
        newErrors[`expiry_date_${index}`] = "Expiry date is required";
      }
    });
        ...
        <form onSubmit={handleSubmit}>
          {licenses.map((license, index) => (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <FormControl fullWidth error={!!errors[`issue_date_${index}`]}>
                  <DatePicker
                    label="Issue Date *"
                    ...
                    slotProps={{
                      textField: {
                        fullWidth: true,
                        error: !!errors[`issue_date_${index}`],
                        helperText: errors[`issue_date_${index}`]
                      },
                    }}
                  />
                </FormControl>

                <FormControl fullWidth error={!!errors[`expiry_date_${index}`]}>
                  <DatePicker
                    label="Expiry Date *"
                    ...
                    slotProps={{
                      textField: {
                        fullWidth: true,
                        error: !!errors[`expiry_date_${index}`],
                        helperText: errors[`expiry_date_${index}`]
                      },
                    }}
                  />
                </FormControl>
              ...
              <TransferList 
                index={index}
                licenseRating={license.license_rating}
